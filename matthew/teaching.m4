...
<p>Use the controls below to increase/decrease the number of terms
\(N\) in the Riemann sum, and alter the \(\alpha\) parameter used to
determine where the function is sampled within each cell.  The value
\(R(\alpha, N)\) of the Riemann sum is updated accordingly.</p>
